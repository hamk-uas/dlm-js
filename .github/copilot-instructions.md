...
- Dependencies: `@hamk-uas/jax-js-nonconsuming` (includes the eslint plugin as a sub-path export) is installed from `github:hamk-uas/jax-js-nonconsuming#5a74c42` (post-v0.8.1: constants → uniform buffers, batched GPU ops, `var<private>` body intermediates, relaxed diamond heuristic, batched copy ops, Phase 4 block_map fusion with proper per-element reduction codegen, outOffset flat gidx fix for Phase 4, cheap-recompute diamond exemption for further dispatch reduction).
