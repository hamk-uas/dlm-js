...
- **Consumer mode (the upgrade & integration loop):** When the user reports a new upstream push to `@hamk-uas/jax-js-nonconsuming`:
  ...
  3. **Run the full test suite**: `pnpm vitest run` — all 317 tests must pass before proceeding (+ 3 skipped under Node — 1 wasm/f32 energy NaN, 2 webgpu-scan no-device).
...
- **Methodology for large tasks:**
  ...
  2. **The ground truth:** Run `pnpm vitest run`. **Test integrity is absolute** — 317 tests + 3 skips across 15 suites, all algorithms (scan/assoc/sqrt-assoc/ud), all device × dtype configs.
...
- Dependencies: `@hamk-uas/jax-js-nonconsuming` (includes the eslint plugin as a sub-path export) is installed from `github:hamk-uas/jax-js-nonconsuming#v0.9.1` (v0.9.1 release: O8c command tape DUS/scatter_add/reverse, scatter_add alignment fix, DISPLAY detection fix; built on v0.9.0).
