...
- **Consumer mode (the upgrade & integration loop):** When the user reports a new upstream push to `@hamk-uas/jax-js-nonconsuming`:
  ...
  3. **Run the full test suite**: `pnpm vitest run` — all 316 tests must pass before proceeding.
...
- **Methodology for large tasks:**
  ...
  2. **The ground truth:** Run `pnpm vitest run`. **Test integrity is absolute** — 316 tests across 14 suites, all algorithms (scan/assoc/sqrt-assoc/ud), all device × dtype configs.
...
- Types & helpers: `src/types.ts` (`StateMatrix`, `CovMatrix` zero-copy wrappers, `DlmFitResult` (includes `p`, `F`), `DlmFitOptions` (includes `F?: number[][]` observation matrix, `obsStd` expanded to `number | ArrayLike<number> | number[][]`), `DlmSmoResult` (includes `p`), `DlmMleOptions`, `DlmForecastResult`, `DlmForecastOptions`, `DlmDtype`, `DlmLossFn`, `DlmParamMeta`, `parseDtype`, `getFloatArrayType`). `adSafeInv` was removed in v0.7.8: all call sites now use `np.linalg.inv` directly (VJP fixed in jax-js-nonconsuming v0.7.8).
