...
- Upstream issues: `issues/` (open issues filed to jax-js-nonconsuming). Key issues: `jax-js-webgpu-laxscan-sequential-dispatch.md` (🟡 mitigated: O(n) backward RTS smoother on WebGPU for `scan` algorithm; `assoc` algorithm uses associativeScan for both forward+backward — Option 2 implemented), `jax-js-missing-jax-primitives.md` (🟢 enhancement: `np.broadcast_to` to replace 20 `np.tile` sites, scalar array indexing to replace `np.dot(vec, mask)` pattern; original `np.where`/matmul items ✅ resolved), `jax-js-wasm-allocator-size-overflow.md` (🔴 open: signed 32-bit overflow in page-count calculation when single allocation > 2 GiB; filed against `297f93a`), `jax-js-jit-cache-bloat.md` (🟡 internal + upstream ergonomics: inline `jit(fn)()` accumulates GPU constants; ~30 slots per `dlmFit` call; workaround: `clearCaches()` periodically; internal fix path: hoist `jit(core)` with shape-based cache), `jax-js-eslint-nested-array-leak.md` (🔴 open: `require-using` rule only visits VariableDeclaration — misses `np.tile(np.reshape(...))` inline temporaries; 3 leaks found in dlm-js), `jax-js-jit-nested-jaxpr-disposal.md` (🔴 open: `ClosedJaxpr.dispose()` is non-recursive — nested scan/assocScan sub-jaxpr constants never freed; `[Symbol.dispose]()` is no-op during PE tracing; 11–29 user-tagged leaks per `dlmMLE` call; workaround: leak budget in tests). Resolved in `65cb449`: qr vmap rule, `[1,m,m]` constant shape under vmap, JIT CodeGenerator stack overflow. Resolved in `d08dd54`: WebGPU fused assocScan shader gidx→eidx rewrite bug (reported after `2b61935`).
