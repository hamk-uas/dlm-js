...
- **Leak detection**: Leak checking runs automatically on every test via `tests/setup.ts` (vitest `setupFiles`). The setup calls `checkLeaks.start()` in `beforeEach` and asserts `checkLeaks.stop().userLeaked` ≤ budget in `afterEach`, so any un-budgeted `np.Array` leak fails the test immediately. The budget defaults to 0 (strict); `mle.test.ts` sets `globalThis.__jaxUserLeakBudget = 30` to accommodate irreducible upstream leaks from `jit(valueAndGrad(...))` anonymous const disposal (see `issues/jax-js-anonymous-const-using-scan.md`). Scripts use `scripts/lib/leak-utils.ts` for the same purpose. No per-test wrapper is needed — just write tests normally and leaks are caught.
