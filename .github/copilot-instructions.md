...
- **Three-branch execution architecture** in `dlmSmo`: (1) Float64 on any device → standard sequential `lax.scan`, (2) Float32 on cpu/wasm → sequential scan + Joseph form covariance update + symmetrization + diagonal clamping, (3) Float32 on webgpu → two-source parallel method: DARE steady-state Kalman gain (forward, classical control theory) + Särkkä & García-Fernández (2020) parallel smoother (backward, Lemmas 5–6 / Theorem 2, arXiv:[1905.13002](https://arxiv.org/abs/1905.13002)), both via `lax.associativeScan` (⌈log₂N⌉+1 GPU dispatches each, Kogge-Stone) + Joseph form. The WebGPU path requires jax-js-nonconsuming v0.7.4 or later (the `fix/jit-scan-einsum-maxargs` fixes are included); see `issues/jax-js-webgpu-jit-einsum.md` for context.
...
- SVG generators: `scripts/gen-niledemo-svg.ts`, `scripts/gen-kaisaniemi-svg.ts`, `scripts/gen-trigar-svg.ts`, `scripts/gen-nile-mle-anim-svg.ts` (accepts variant: scan/assoc/webgpu; + `scripts/collect-nile-mle-frames.ts` runs both scan + assocScan variants), `scripts/gen-energy-mle-anim-svg.ts` (accepts variant: scan/assoc/webgpu; + `scripts/collect-energy-mle-frames.ts` runs both variants), `scripts/gen-ozone-svg.ts`, `scripts/gen-missing-svg.ts` (missing-data demo with NaN interpolation and ystd widening). WebGPU MLE variant is placeholder-only: blocked by upstream jax-js tracer disposal error (both `jit(valueAndGrad)` and bare `valueAndGrad` compute correctly in v0.7.4 but throw a tracer disposal error after returning the result); see `issues/jax-js-webgpu-mle-backward-buffer-limit.md`.
...
- Upstream issues: `issues/` (precision analysis, WebGPU JIT einsum bug filed to jax-js-nonconsuming). Key open issues: `jax-js-webgpu-mle-backward-buffer-limit.md` (MLE on WebGPU blocked — v0.7.4: `jit(valueAndGrad)` and bare `valueAndGrad` both compute correctly but throw a tracer disposal error after the result is returned; buffer limit (v0.7.1–3) and concatenate VJP (v0.7.2) are fixed), `jax-js-webgpu-laxscan-sequential-dispatch.md` (O(n) backward RTS smoother, architectural).
