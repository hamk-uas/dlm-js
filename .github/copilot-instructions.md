...
- **Three-branch execution architecture** in `dlmSmo`: (1) Float64 on any device → standard sequential `lax.scan`, (2) Float32 on cpu/wasm → sequential scan + Joseph form covariance update + symmetrization + diagonal clamping, (3) Float32 on webgpu → two-source parallel method: DARE steady-state Kalman gain (forward, classical control theory) + Särkkä & García-Fernández (2020) parallel smoother (backward, Lemmas 5–6 / Theorem 2, arXiv:[1905.13002](https://arxiv.org/abs/1905.13002)), both via `lax.associativeScan` (⌈log₂N⌉+1 GPU dispatches each, Kogge-Stone) + Joseph form. The WebGPU path requires the `fix/jit-scan-einsum-maxargs` branch of jax-js-nonconsuming (commit f09121d or later); see `issues/jax-js-webgpu-jit-einsum.md` for full context.
...
- Upstream issues: `issues/` (precision analysis, WebGPU JIT einsum bug filed to jax-js-nonconsuming). Key open issues: `jax-js-webgpu-mle-backward-buffer-limit.md` (MLE on WebGPU blocked — v0.7.1: `jit(valueAndGrad)` over `lax.associativeScan` exceeds 8-buffer bind group limit; v0.7.2: `Nonlinear operation in backward pass for concatenate` on both jit and non-jit paths), `jax-js-webgpu-laxscan-sequential-dispatch.md` (O(n) backward RTS smoother, architectural).
