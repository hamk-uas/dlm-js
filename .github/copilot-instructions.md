...
- Types & helpers: `src/types.ts` (`StateMatrix`, `CovMatrix` zero-copy wrappers, `DlmFitResult` (includes `p`, `F`), `DlmFitOptions` (includes `F?: number[][]` observation matrix, `obsStd` expanded to `number | ArrayLike<number> | number[][]`), `DlmSmoResult` (includes `p`), `DlmMleOptions`, `DlmForecastResult`, `DlmForecastOptions`, `DlmDtype`, `DlmAlgorithm`, `DlmStabilization` (union: `'matlab' | 'none' | DlmStabilizationFlags`), `DlmStabilizationPreset`, `DlmStabilizationFlags`, `DlmLossFn`, `DlmParamMeta`, `parseDtype`, `getFloatArrayType`). `adSafeInv` was removed in v0.7.8: all call sites now use `np.linalg.inv` directly (VJP fixed in jax-js-nonconsuming v0.7.8).
