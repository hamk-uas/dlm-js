...
- **Three-branch execution architecture** in `dlmSmo`: (1) Float64 on any device → standard sequential `lax.scan`, (2) Float32 on cpu/wasm → sequential scan + Joseph form covariance update + symmetrization + diagonal clamping, (3) Float32 on webgpu → two-source parallel method: DARE steady-state Kalman gain (forward, classical control theory) + Särkkä & García-Fernández (2020) parallel smoother (backward, Lemmas 5–6 / Theorem 2, arXiv:[1905.13002](https://arxiv.org/abs/1905.13002)), both via `lax.associativeScan` (⌈log₂N⌉+1 GPU dispatches each, Kogge-Stone) + Joseph form. The WebGPU path requires jax-js-nonconsuming v0.7.3 or later (the `fix/jit-scan-einsum-maxargs` fixes are included); see `issues/jax-js-webgpu-jit-einsum.md` for context.
...
- SVG generators: `scripts/gen-niledemo-svg.ts`, `scripts/gen-kaisaniemi-svg.ts`, `scripts/gen-trigar-svg.ts`, `scripts/gen-nile-mle-anim-svg.ts` (accepts variant: scan/assoc/webgpu; + `scripts/collect-nile-mle-frames.ts` runs both scan + assocScan variants), `scripts/gen-energy-mle-anim-svg.ts` (accepts variant: scan/assoc/webgpu; + `scripts/collect-energy-mle-frames.ts` runs both variants), `scripts/gen-ozone-svg.ts`, `scripts/gen-missing-svg.ts` (missing-data demo with NaN interpolation and ystd widening). WebGPU MLE variant is placeholder-only: blocked by upstream jax-js buffer limit (12 buffers in backward pass vs 8-buffer WebGPU bind group limit); see `issues/jax-js-webgpu-mle-backward-buffer-limit.md`.
...
- Upstream issues: `issues/` (precision analysis, WebGPU JIT einsum bug filed to jax-js-nonconsuming). Key open issues: `jax-js-webgpu-mle-backward-buffer-limit.md` (MLE on WebGPU blocked — v0.7.3: `jit(valueAndGrad)` still exceeds 8-buffer bind group limit (9 buffers); bare `valueAndGrad` now computes correctly but hits tracer disposal after result; v0.7.2 `Nonlinear operation in backward pass for concatenate` regression is fixed in v0.7.3), `jax-js-webgpu-laxscan-sequential-dispatch.md` (O(n) backward RTS smoother, architectural).
...
- Dependencies: `@hamk-uas/jax-js-nonconsuming` v0.7.3 (includes the eslint plugin as a sub-path export) is installed from `github:hamk-uas/jax-js-nonconsuming#v0.7.3`.
