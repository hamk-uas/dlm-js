...
- **UD epsilon isolation rule** (promoted from mistake ledger, count=2): When regularization ε is added for Cholesky PSD guarantee, compute ALL downstream quantities (gain K, covariance time update) from the exact covariance (`C_true` in carry), never from ε-contaminated Bierman/LDL factorization products. ε goes ONLY into the Cholesky input. Violation symptom: dtype-invariant errors (structural bug, not FP drift).
