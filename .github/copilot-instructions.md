...
- Dependencies: `@hamk-uas/jax-js-nonconsuming` (includes the eslint plugin as a sub-path export) is installed from `github:hamk-uas/jax-js-nonconsuming#8501de9` (post-v0.8.0: constants → uniform buffers, batched GPU ops, `var<private>` body intermediates, relaxed diamond heuristic, batched copy ops, Phase 4 block_map fusion, per-element reduction guard).
