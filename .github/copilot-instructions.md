...
- **Three-branch execution architecture** in `dlmSmo`: (1) Float64 on any device → standard sequential `lax.scan`, (2) Float32 on cpu/wasm → sequential scan + Joseph form covariance update + symmetrization + diagonal clamping, (3) Float32 on webgpu → parallel method from Särkkä & García-Fernández (2020, arXiv:[1905.13002](https://arxiv.org/abs/1905.13002)): currently uses DARE steady-state Kalman gain for the forward filter (classical control theory, **planned to be replaced** with the exact 5-tuple parallel forward filter from Lemmas 1–2 of the same paper) + exact parallel backward smoother (Lemmas 5–6 / Theorem 2), both via `lax.associativeScan` (⌈log₂N⌉+1 GPU dispatches each, Kogge-Stone) + Joseph form. **Note**: the backward smoother already uses the paper’s exact formulation; the forward filter was independently approximated via DARE when Lemmas 1–2 were overlooked. See `issues/dare-to-exact-parallel.md` for the refactor plan. The WebGPU path requires jax-js-nonconsuming v0.7.4 or later (the `fix/jit-scan-einsum-maxargs` fixes are included); see `issues/jax-js-webgpu-jit-einsum.md` for context.
...
- MLE parameter estimation: `src/mle.ts` (`dlmMLE`: autodiff-based MLE via `jit(valueAndGrad + optax Adam)`, AD-safe `buildDiagW`, `buildG` for AR coefficient estimation). Two loss paths: `makeKalmanLoss` (sequential `lax.scan`, CPU/WASM) and `makeKalmanLossAssoc` (`lax.associativeScan`, WebGPU+Float32; currently uses traced DARE for K_ss, **planned: exact 5-tuple forward filter from [1, Lemmas 1–2]**, eliminating the DARE approximation and its ~1–2% bias). `makeKalmanLossAssoc` currently unrolls 50 Riccati iterations inside the AD tape so K_ss depends on theta; uses Joseph form in DARE; heterogeneous V² uses `mean(V²)` for DARE, per-step V²(t) in scan elements and likelihood; float-mask blending for NaN (not boolean np.where). Supports `fitar: true` for AR coefficient estimation.
...
- SVG generators: `scripts/gen-niledemo-svg.ts`, `scripts/gen-kaisaniemi-svg.ts`, `scripts/gen-trigar-svg.ts`, `scripts/gen-nile-mle-anim-svg.ts` (accepts variant: scan/assoc/webgpu; + `scripts/collect-nile-mle-frames.ts` runs both scan + assocScan variants), `scripts/gen-energy-mle-anim-svg.ts` (accepts variant: scan/assoc/webgpu; + `scripts/collect-energy-mle-frames.ts` runs both variants), `scripts/gen-ozone-svg.ts`, `scripts/gen-missing-svg.ts` (missing-data demo with NaN interpolation and ystd widening). WebGPU MLE variant is placeholder-only: blocked by upstream jax-js issues (now resolved in v0.7.4 — `jit(valueAndGrad)` over `lax.associativeScan` works correctly); see `issues/jax-js-webgpu-mle-backward-buffer-limit.md`. TODO: implement the full WebGPU MLE path.
...
- Upstream issues: `issues/` (precision analysis, WebGPU JIT einsum bug filed to jax-js-nonconsuming). Key resolved/open issues: `jax-js-webgpu-mle-backward-buffer-limit.md` (✅ resolved in v0.7.4 — `jit(valueAndGrad)` over `lax.associativeScan` now works on WebGPU; the `Referenced tracer Array has been disposed` error seen during testing was a repro bug from calling `dispose()` after `consumeData()`), `jax-js-webgpu-laxscan-sequential-dispatch.md` (O(n) backward RTS smoother, architectural).
...
- **Three-branch architecture**: `dlmSmo` selects Float64 (no stabilization), cpu/wasm+Float32 (Joseph form), or webgpu+Float32. The WebGPU path currently uses a **two-source parallel method**: (a) forward: DARE steady-state Kalman gain (classical control theory, MATLAB convention: A_ss = G−K_ss·F) + `lax.associativeScan` prefix scan; ~1–2% early-step approximation (**planned: replace with exact 5-tuple forward filter from [1, Lemmas 1–2]** — the DARE approximation was independently invented when these lemmas were overlooked in the cited paper), (b) backward: exact per-timestep smoother gains E_k from Särkkä & García-Fernández (2020, arXiv:[1905.13002](https://arxiv.org/abs/1905.13002), Lemmas 5–6, Theorem 2) via batched `np.linalg.inv`, composed by `lax.associativeScan(compose, elems, { reverse: true })` suffix scan; no accuracy loss. WASM is ~10–20× faster than CPU for small-matrix dlmFit. Both WebGPU scans dispatch ⌈log₂N⌉+1 GPU kernels (architecturally optimal; WebGPU lacks cross-workgroup sync so Kogge-Stone requires a separate dispatch per round). Caveat: for purely elementwise `fn` bodies, `associativeScan` achieves exactly 1 GPU kernel per round; if `fn` contains reductions (matmul, einsum with inner sum), each round produces additional dispatches. See `issues/dare-to-exact-parallel.md` for the refactor plan.
...
- Dependencies: `@hamk-uas/jax-js-nonconsuming` v0.7.4 (includes the eslint plugin as a sub-path export) is installed from `github:hamk-uas/jax-js-nonconsuming#v0.7.4`.
