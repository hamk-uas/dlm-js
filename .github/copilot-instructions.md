...
- Upstream issues: `issues/` (precision analysis, WebGPU JIT einsum bug filed to jax-js-nonconsuming). Key open issues: `jax-js-webgpu-mle-backward-buffer-limit.md` (MLE on WebGPU blocked — `jit(valueAndGrad)` over `lax.associativeScan` exceeds 8-buffer bind group limit), `jax-js-webgpu-laxscan-sequential-dispatch.md` (O(n) backward RTS smoother, architectural).
