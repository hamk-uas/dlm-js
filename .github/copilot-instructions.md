...
- **Three-branch execution architecture** in `dlmSmo`: (1) Float64 on any device → standard sequential `lax.scan`, (2) Float32 on cpu/wasm → sequential scan + Joseph form covariance update + symmetrization, (3) Float32 on webgpu → exact parallel method from Särkkä & García-Fernández (2020, arXiv:[1905.13002](https://arxiv.org/abs/1905.13002)): exact 5-tuple forward filter (Lemmas 1–2, per-timestep Kalman gains) + exact parallel backward smoother (Lemmas 5–6 / Theorem 2), both via `lax.associativeScan` (⌈log₂N⌉+1 GPU dispatches each, Kogge-Stone) + Joseph form. The WebGPU path requires jax-js-nonconsuming v0.7.4 or later (project uses v0.7.7).
...
- Types & helpers: `src/types.ts` (TypedArray usage, `getFloatArrayType`, `DlmForecastResult` interface, `adSafeInv` — AD-safe batched matrix inverse for all m: analytic cofactor expansion for m ≤ 3, recursive Schur-complement block inversion for m ≥ 4; works around broken `np.linalg.inv` VJP in jax-js).
