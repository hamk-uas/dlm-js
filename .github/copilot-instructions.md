...
- **Provider mode (writing for the downstream agent):** When you modify our public API (`dlmFit`, `dlmMLE`, `dlmForecast`, `dlmGenSys`, `dlmPrior`, types in `src/types.ts`), provide a Migration Bundle:
  ...
  2. **Workaround signatures (the "debt collector"):** Explicitly identify patterns consumers likely used to bypass old limitations. Provide migration guidance in the commit message. If you create a new release version in GitHub, include these instructions in the version description. *Example:* "If the consumer wraps `dlmFit` in a retry loop because Float32 covariance diverged, that's fixed by Joseph form — delete the retry."
...
- **Consumer mode (the upgrade & integration loop):** When the user reports a new upstream push to `@hamk-uas/jax-js-nonconsuming`:
  ...
  9. **Regenerate ALL artifacts** including WebGPU: upstream changes affect all backends. Run `pnpm run bench:wasm` for WASM artifacts, then collect WebGPU MLE frames via `pnpm run gen:svg:nile-mle-anim && pnpm run gen:svg:energy-mle` (WebGPU parts run in Chromium via Playwright), and run `pnpm run bench:full`. Finish with `pnpm run check:freshness` to confirm zero stale artifacts.
...
- **Upstream-blocked mode (regressions that prevent commit):** When an upstream upgrade introduces regressions that break existing tests:
  ...
  5. **Clean up resolved issues.** A blocked upgrade may still fix *some* previously-filed issues. For each open issue in `issues/`, run its repro against the new (uncommitted) version. If a repro passes, delete the issue file and repro script, update `copilot-instructions.md` references, and commit: `git add -A issues/ .github/ && git commit -m "issues: resolve <topic>"`. Do this *before* reverting the lockfile so the repros run against the candidate version.
  6. **Do not regenerate timing artifacts.** Benchmarks, SVGs, and timing sidecars should not be refreshed until the upgrade can land cleanly. Partial refreshes create inconsistent artifacts.
  7. **Inform the user** that issues are ready for relay. The user passes the `issues/` bundle to the upstream team. The agent waits for a fixed upstream push before retrying the upgrade via Consumer mode.
  8. **Track blocked state** in `/memories/repo/task-notes.md` under "In progress" — record which upstream commit is blocked, which issues are filed, and what internal changes were applied separately.
...
- **Methodology for large tasks:**
  ...
  2. **The ground truth:** Run `pnpm vitest run`. **Test integrity is absolute** — 318 tests across 14 suites, all algorithms (scan/assoc/sqrt-assoc/ud), all device × dtype configs.
...
- dlm-js is a TypeScript Kalman filter + RTS smoother library using the `@hamk-uas/jax-js-nonconsuming` numeric backend, inspired by the MATLAB Dynamic Linear Model (DLM). It extends the MATLAB original with autodiff-based MLE (`jit(valueAndGrad + Adam)`) and an exact O(log N) parallel filter+smoother via `lax.associativeScan` (Särkkä & García-Fernández 2020). The library exposes CPU/WASM/WebGPU-capable implementations via `dlmFit` (options-bag API: `dlmFit(y, opts: DlmFitOptions)`); `dlmSmo` is an internal function (not exported). The WebGPU path is a research/experimental backend — dispatch overhead makes it slower than WASM for typical dataset sizes. `dlmForecast(fit, h, opts?)` propagates the last smoothed state h steps ahead with no new observations, returning `yhat`, `ystd` (monotonically growing), full state/covariance trajectories, and optional covariate support; `obsStd` defaults to `fit.obsNoise[0]` (override via `opts.obsStd`). `dlmGenSys` generates G/F state-space matrices for polynomial trend, seasonal, and AR components. `dlmMLE(y, opts?)` estimates observation noise, state noise, and optionally AR coefficients via autodiff (`jit(valueAndGrad + optax Adam)`, `fitAr: true` for AR coefficient estimation). Supports MAP estimation via `loss` option (custom `DlmLossFn` callback receiving natural-scale params + `DlmParamMeta`) or the `dlmPrior` factory (Inverse-Gamma on variances, Normal on AR coefficients — matches MATLAB/R DLM `dlmGibbsDIG` conventions). All public functions use options-bag signatures.
- **Multivariate observations (p > 1)**: `dlmFit` accepts `y: number[][]` (n × p) and `F: number[][]` (p × m observation matrix) in `DlmFitOptions`. The filter/smoother branches on `p === 1` at JS trace time so JIT constant-folds the p=1 path (zero overhead). p > 1 uses matrix `inv`/`matmul`/`einsum` in forward/backward steps and `slogdet` for the likelihood. NaN masking is all-or-nothing per timestep (scalar mask broadcasts). Guards: `sqrt-assoc` and `dlmForecast` throw for p > 1. MLE (`dlmMLE`) currently supports p = 1 only.
- **Three-branch execution architecture** in `dlmSmo`: (1) Float64 on any device → sequential `lax.scan` + `triu(C)+triu(C,1)'` covariance symmetrization by default (matches MATLAB `dlmsmo.m`; reduces max relative error vs Octave from ~2e-9 to ~4e-12; disable with `stabilization:'none'`), (2) Float32 on cpu/wasm → sequential scan + Joseph form covariance update + symmetrization + unconditional `C += 1e-6·I`, (3) Float32 on webgpu → exact parallel method from Särkkä & García-Fernández (2020, arXiv:[1905.13002](https://arxiv.org/abs/1905.13002)): exact 5-tuple forward filter (Lemmas 1–2, per-timestep Kalman gains) + exact parallel backward smoother (Lemmas 3–4 / Theorem 2), both via `lax.associativeScan` (⌈log₂N⌉+1 GPU dispatches each, Kogge-Stone) + Joseph form. A fourth path, `algorithm: 'sqrt-assoc'`, implements the square-root parallel smoother (Yaghoobi et al. 2022, arXiv:2207.00426) in Cholesky-factor space — covariances are stored as their Cholesky factors (U, Z, D) and composed via `tria()` blocks, ensuring PSD by construction. The sqrt-assoc path uses proper QR-based `tria(A) = R'` where `[Q, R] = np.linalg.qr(A')`, plus `lax.linalg.triangularSolve` for triangular solves — no condition-number squaring, works for all state dimensions (including fullSeasonal m=13). The WebGPU path requires jax-js-nonconsuming v0.8.0 or later; earlier commit `2b61935` had a fused Kogge-Stone shader bug (gidx→eidx rewrite was a no-op) that caused numerical garbage — fixed in `d08dd54`.
- The authoritative numerical reference is generated by Octave (`tests/octave/niledemo.m`, `tests/octave/gensys_tests.m`, `tests/octave/kaisaniemi_demo.m`, `tests/octave/ozonedemo.m`, `tests/octave/gappeddata_test.m`, `tests/octave/multivariate_demo.m`) and compared in `tests/niledemo.test.ts`, `tests/gensys.test.ts`, `tests/ozone.test.ts`, `tests/gapped.test.ts`, and `tests/multivariate.test.ts`. Synthetic ground-truth tests in `tests/synthetic.test.ts` verify against mathematically known true states.

Quick start — commands you will need (copy/paste) ▶️
- Install: `pnpm install`.
- Run node tests (uses source TS): `pnpm vitest run` or `pnpm run test:node`.
- Run WebGPU tests (Chromium via Playwright): `pnpm run test:gpu` (NVIDIA) or `GPU=intel pnpm run test:gpu` (Intel Arc). Config: `tests/gpu-config.ts`, runner: `scripts/gpu-test.sh`.
...
- Generate fit-demo SVGs only (WASM-only, no GPU needed): `pnpm run gen:svg:fit` (nile, kaisaniemi, trigar, ozone, gapped — scan + assoc variants each).
...
- MLE parameter estimation: `src/mle.ts` (`dlmMLE`: autodiff-based MLE via `jit(valueAndGrad + optax Adam)`, AD-safe `buildDiagW`, `buildG` for AR coefficient estimation). Two optimizer paths: `optimizer:'natural'` (default for f64, second-order Newton / Fisher scoring with FD Hessian + Levenberg-Marquardt damping) and `optimizer:'adam'` (default for f32, first-order via optax Adam). Two loss paths: `makeKalmanLoss` (sequential `lax.scan`, CPU/WASM) and `makeKalmanLossAssoc` (`lax.associativeScan`, WebGPU+Float32; uses exact 5-tuple forward filter from [1, Lemmas 1–2] with per-timestep Kalman gains, regularized inverse + push-through identity in compose). NaN masking uses `np.where(bool_mask, x, y)` for blend patterns (AD-safe since jax-js `297f93a`); remaining scalar masks use float-mask multiply. `np.concatenate` has working VJP since jax-js v0.7.8, used for prediction assembly and natural-scale param construction. Supports `fitAr: true` for AR coefficient estimation.
...
- Types & helpers: `src/types.ts` (`StateMatrix`, `CovMatrix` zero-copy wrappers, `DlmFitResult` (includes `p`, `F`), `DlmFitOptions` (includes `F?: number[][]` observation matrix, `obsStd` expanded to `number | ArrayLike<number> | number[][]`), `DlmSmoResult` (includes `p`), `DlmMleOptions`, `DlmForecastResult`, `DlmForecastOptions` (includes optional `obsStd`, `timestamps`), `DlmModelSpec` (shared model fields: order, harmonics, seasonLength, fullSeasonal, arCoefficients, spline — extended by DlmFitOptions, DlmMleOptions, DlmOptions), `DlmDtype`, `DlmAlgorithm`, `DlmStabilization` (union: `'matlab' | 'none' | DlmStabilizationFlags`), `DlmStabilizationPreset`, `DlmStabilizationFlags`, `DlmLossFn`, `DlmParamMeta`, `parseDtype`, `getFloatArrayType`). `adSafeInv` was removed in v0.7.8: all call sites now use `np.linalg.inv` directly (VJP fixed in jax-js-nonconsuming v0.7.8).
...
- Upstream issues: `issues/` (open issues filed to jax-js-nonconsuming). Resolved in `e034f26`: drain PETracer #rc in partialEvalGraphToJaxpr for multi-output scan equations (was `jax-js-anonymous-const-using-scan.md` — remaining 7 MAP/prior test failures were dlm-js disposal bugs, not upstream). Resolved in `41b0bda`+`a9db43d`: nested jaxpr disposal (was `jax-js-jit-nested-jaxpr-disposal.md` — zero leaks in repro). Resolved in `99ca222`: evalJaxpr non-consuming refactor (removes move-semantics `.ref` patterns). Resolved in `65cb449`: qr vmap rule, `[1,m,m]` constant shape under vmap, JIT CodeGenerator stack overflow. Resolved in `d08dd54`: WebGPU fused assocScan shader gidx→eidx rewrite bug (reported after `2b61935`). Resolved in `9e25ae7`: ESLint `no-nested-array-leak` rule (was `jax-js-eslint-nested-array-leak.md`). Resolved in `6e6d4fe`: `lax.dynamicIndexInDim`/`lax.sliceInDim` scalar array indexing. Resolved in `d73ce67`: jit() dedup by function identity (was `jax-js-jit-cache-bloat.md`). Resolved in `bb126c6`+`a6d37da`: WASM allocator int32/signed-shift overflow above 2 GiB (was `jax-js-wasm-allocator-size-overflow.md`). Resolved in `26c6a48`: WebGPU scan fallback batching (was `jax-js-webgpu-laxscan-sequential-dispatch.md`, 🟡 mitigated — O(N) architecture is a WebGPU limitation). Resolved in `76a004f`: WebGPU compiled-loop Y≠carry data aliasing (was `jax-js-webgpu-laxscan-accuracy.md` — carry output JitIds not registered in `jitIdToScanGid`, introduced by `0e5a982`). Mitigated in `10ae2aa`: constants → uniform buffers unblocks 5-tuple block-map on WebGPU (was `jax-js-webgpu-block-map-perf-regression.md`, 🟡 — GPU/WASM ratio ~6× warm at N=100, unchanged through v0.11.0). Resolved in `2554290`: WebGPU reverse assocScan stride-32 boundary artifact — `var<workgroup>` body intermediates caused 256-thread race condition in Kogge-Stone compose; fix: `var<private>` (was `jax-js-webgpu-reverse-assocscan-boundary.md`). Improved in `a2062fa`: batch all GPU ops in assocScan block-map execution via `beginBatch()`/`endBatch()` for single `queue.submit()`. Improved in `53ae2e7`: relax diamond heuristic for view ops in splitGraphDataflow — 19% fewer kernel dispatches, ~9% faster. Improved in `33ded59`: batch copy operations (DUS, scatter_add, reverse, fori_loop) in JIT execute steps — near-flat N-scaling for WebGPU assocScan. Improved in `4fd9f8d`: fuse assocScan Phase 4 into single block_map dispatch — eliminates O(M) per-block execute calls (18.6× speedup at N=204,800). Resolved in `8501de9`: Phase 4 fused block_map per-element reduction guard — `bodyHasReductions` check falls back to `mapOverBlocks` when body contains reduction kernels (matmul/einsum); fixes multi-field reverse assocScan corruption for m≥5 (was `jax-js-webgpu-reverse-assocscan-multifield.md`). Improved in `66670f4`: proper per-element reduction & multi-element codegen in fused block_map shader — re-enables fused Phase 4 for bodies with reductions (removed `bodyHasReductions` guard), three codegen paths (workgroup tree / per-element reduction / elementwise multi-element). Mitigated in `df765e9`: outOffset flat gidx for Phase 4 — fixes main multi-field reverse assocScan corruption for m≥5 (n=120✅, n=128✅); residual stub-block corruption remained when N mod blockSize is small. Resolved in `0edd5e9`: Dot vmap batch axis fix (was at ndim-2 instead of position 0) + subgroup guard `sg_size >= blockSize` + fallback gridOffset fix — all stub-block cases pass (n=65✅, n=128✅, n=200✅; was `jax-js-webgpu-reverse-assocscan-multifield.md`). Fixed in `5a19b0b`: integer dtype in fused WAS shader bindings — u32/i32 associative scans silently failed. Improved in `5a74c42`: cheap-recompute diamond exemption — unary elementwise, binary+lit, where+2-lit ops skip forced materialization and fuse into downstream reduction epilogues, reducing dispatch count. Improved in `7c5aadf`: O2 scalar promotion — `pushLit` embeds constant scalar data in malloc step via `initialData`, eliminating zero-input kernel dispatches (writeBuffer/memcpy instead of compute shader). Added in `4e7206fb`: dispatch counter (`WebGPUBackend.dispatchCount` getter + `resetDispatchCount()`). Released as v0.8.2 (`e6891a3`). Released as v0.8.3 (`b478a38`): squash of ~17 commits — command tape O8a/O9b, analytical Cholesky n≤4, Decoupled Fallback scan, subgroupShuffleUp/InclusiveAdd P8, profileGpu P9/T3, adapterArchitecture fix. Released as v0.8.4 (`cc53907`): Dot vmap batch fix (`0edd5e9`), integer dtype WAS shader (`5a19b0b`), version bump. Released as v0.9.0 (`56286d0`): colored multi-slab arena allocator (O9a-v2), constants slab + conflict-graph coloring (O9c), deterministic GPU teardown, test helpers removed from public API. Released as v0.9.1 (`44b31d3`): O8c command tape DUS/scatter_add/reverse, scatter_add alignment fix, DISPLAY detection fix. Released as v0.9.2 (`d174a18d`): leaf packing for 6+ tuple associative scan — packs all leaves into 2 flat storage buffers with compile-time byte offsets to avoid exceeding `maxStorageBuffersPerShaderStage` binding limit; also includes flushBatch mid-batch submit, 16-byte malloc floor, command-tape simplified uniform buffer teardown. Resolves O8c DUS CopyBufferToBuffer size mismatch for sqrt-assoc m≤2 (was `jax-js-webgpu-o8c-dus-sqrt-assoc.md`); remaining sqrt-assoc m=1 precision issue on WebGPU is a dlm-js algorithmic problem (ystd/xstd ~1-6× off vs Octave for order=0 m=1), not upstream. Released as v0.9.3 (`ea264386`): WebGPU scan carry/Y source classification + `Backend.fence()` API (no zero-byte-read overhead in `blockUntilReady`), command tape scan dispatches, conditional nan/inf WGSL shader helpers, ESLint `no-nested-array-leak` autofix, upstream cherry-picks (triangularSolve JVP fix `2c17ffbb`, inv/matrixPower dtype fix `93649f32`, WGSL float→int cast fix `6b724951`), new numpy functions (round, rint, copysign, cross, vecmat, matvec, arrayEqual, arrayEquiv, average, Array.view, logical boolean ops), conv2d BlockMap rewrite with halo support, fluid simulation demo + WebGPU interop (getWebGPUDevice, gpuBuffer, gpuBufferSync). Released as v0.9.4: GPU calibration at init, ForiLoop block-map tile feasibility fix, captureJitReport/formatJitReport API. Released as v0.9.5: major ownership restructuring (phantom-ref deletion → explicit creation-ref balancing), disposePeIntermediates, WASM worker threads, SIMD cast instructions. Released as v0.9.6: bitwise ops, vectorNorm, QR test shortcuts removed, FEATURES matrix repaired. Released as v0.9.7 (`503b7636`): fix QR foriLoop arange disposal — gates `arange_m`/`arange_n` disposal in `householderQR2D` on tracing context (was `jax-js-qr-foriloop-arange-disposal.md` — UseAfterFreeError on int32 arrays when `np.linalg.qr` called inside `jit()` on matrices with dim > 8). Released as v0.10.0 (`b95049b6`): Conv2d fused-shader path on WebGPU. Released as v0.10.1 (`35e51f0a`): SharedArrayBuffer guard in WASM `#readData`, conv halo dedup. Released as v0.11.0 (`dbff3eed`): ND `dynamicUpdateSlice` with static per-axis offsets + `array-manipulation-patterns.md` doc + foriLoop autodiff/dynamic-bound limitations docs.
...
- **UD epsilon isolation rule** (promoted from mistake ledger, count=2; now resolved by Thornton time update): The Thornton MWGS time update propagates UD factors directly without Cholesky re-factorization, eliminating the epsilon accumulation problem entirely. W_t is handled by direct diagonal extraction (`D_W = einsum('ii->i', W_t)`) — no Cholesky of W, no ε contamination even when w[i]=0. The only remaining epsilon is in the initial C0 Cholesky (one-time, non-accumulating). Historical context: the previous Cholesky→LDL re-factorization path required a `C_true` carry field to isolate ε from backward smoother outputs.
- **Bierman D-update cancellation rule**: In the Bierman measurement update column loop, the D ratio must be computed as `α_old / α` directly — never as `(α − f·g) / α`. The latter suffers catastrophic cancellation in f32 when `f·g ≫ α_old` (e.g. energy-in.json with 15 625× dynamic range in W diagonal). This caused NaN divergence on WASM/WebGPU f32 before the fix.
- Dependencies: `@hamk-uas/jax-js-nonconsuming` (includes the eslint plugin as a sub-path export) is installed from `github:hamk-uas/jax-js-nonconsuming#v0.11.0` (v0.11.0 release: ND `dynamicUpdateSlice` with static per-axis offsets, `array-manipulation-patterns.md` doc; built on v0.10.1 — SharedArrayBuffer guard, conv halo dedup; v0.10.0 — Conv2d fused-shader WebGPU; v0.9.7 — QR foriLoop arange disposal fix; v0.9.6 — bitwise ops, vectorNorm; v0.9.5 — ownership restructuring, WASM worker threads, SIMD; v0.9.4 — GPU calibration, captureJitReport).
...
- **Fifteen test suites**: `niledemo.test.ts` (8 tests, Nile data vs Octave), `gensys.test.ts` (47 tests, multi-model vs Octave), `synthetic.test.ts` (24 tests, known true states), `mle.test.ts` (21 tests, MLE parameter & AR coefficient estimation + natural gradient optimizer × scan/assoc, MAP/dlmPrior factory, including gapped data, on WASM), `covariate.test.ts` (5 tests, X parameter / β recovery), `ozone.test.ts` (2 tests, ozone demo smoke), `forecast.test.ts` (6 tests, h-step-ahead forecasting), `gapped.test.ts` (16 tests, order=1 and order=0 with NaN observations vs Octave reference), `assocscan.test.ts` (24 tests, assoc path validated against same Octave references on wasm/f64 — covers all gensys models, Nile demo, and gapped data), `timestamps.test.ts` (23 tests, irregular timestamps including AR with integer Δt), `sqrtassoc.test.ts` (24 tests: wasm/f64 precision comparison + wasm/f32 smoke-only all-outputs-finite; all models including fullSeasonal m=13), `multivariate.test.ts` (48 tests: p=2 vector observations vs Octave — order=1, order=0, gapped, order=1+AR(2) × scan/assoc × all backends), `ud.test.ts` (48 tests: UD/Bierman/Thornton algorithm validated against Octave — f64 precision + f32 smoke for all models, niledemo, and gapped data), `options.test.ts` (22 tests, unknown-option validation for all public API functions), `webgpu-scan.test.ts` (15 tests: 5 models × {scan, ud, sqrt-assoc} — WebGPU accuracy regression guard; excluded from Node config, exercises via `pnpm run test:gpu` using Chromium/Playwright; thresholds from good-era commit `90a9e77`). Node test suite: 318 tests (318 pass). WebGPU suite: 15 tests (14 pass + 1 skip: sqrt-assoc m=1 precision).
...
- **Float32 + m > 2**: the Joseph form + `C += 1e-6·I` stabilization prevents divergence (0 crashes across all 256 flag combos in exhaustive search), but precision is limited to ~1e-2 relative error — these combinations are skipped for precision comparison in `gensys.test.ts` and `synthetic.test.ts` (smoke-test only).
- **Leak detection**: Leak checking is handled globally by `tests/setup.ts` via `beforeEach`/`afterEach` hooks that wrap every test in `checkLeaks.start()`/`stop()`. No per-call wrapper is needed in tests. Scripts use `scripts/lib/leak-utils.ts` for their own `withLeakCheck` helper.
...
- **Error metric reports 0% — don't trust it**: (1) Check for NaN values first (`assertAllFinite`) — NaN > max is always false, silently ignored. (2) Use `ArrayBuffer.isView(x) || Array.isArray(x)` when traversing comparison data — `Array.isArray(Float32Array)` returns false, so walk() never enters TypedArrays.
...
6. `src/types.ts` (`StateMatrix`, `CovMatrix` zero-copy wrappers, `DlmFitResult`, `DlmFitOptions`, `DlmMleOptions`, `DlmForecastResult`, `DlmForecastOptions`, `DlmModelSpec`, `DlmDtype`, `DlmLossFn`, `DlmParamMeta`, `parseDtype`)
...
12. `tests/multivariate.test.ts` (p > 1 vector observations vs Octave reference)
