...
- **Consumer mode (the upgrade & integration loop):** When the user reports a new upstream push to `@hamk-uas/jax-js-nonconsuming`:
  ...
  3. **Run the full test suite**: `pnpm vitest run` — all 248 tests must pass before proceeding.
...
- **Methodology for large tasks:**
  ...
  2. **The ground truth:** Run `pnpm vitest run`. **Test integrity is absolute** — 248 tests across 12 suites, both algorithms, all device × dtype configs.
...
- **UD epsilon isolation rule** (promoted from mistake ledger, count=2; now resolved by Thornton time update): The Thornton MWGS time update propagates UD factors directly without Cholesky re-factorization, eliminating the epsilon accumulation problem entirely. The only remaining epsilon is in the initial C0 Cholesky (one-time, non-accumulating) and W_t Cholesky (user-specified, well-conditioned). Historical context: the previous Cholesky→LDL re-factorization path required a `C_true` carry field to isolate ε from backward smoother outputs.
