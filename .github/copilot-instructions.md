...
- dlm-js is a computation-only TypeScript port of a MATLAB Dynamic Linear Model (DLM) using the `@jax-js-nonconsuming/jax` numeric backend. The library implements Kalman filter + RTS smoother and exposes CPU/WASM/WebGPU-capable implementations via `dlmFit`/`dlmSmo` (modes: `for`, `scan`, `jit`).
...
- Install: `pnpm install` (project expects a local `../jax-js` during active development — package.json uses `link:../jax-js`).
...
- Local dependency: `@jax-js-nonconsuming/jax` is installed from `github:hamk-uas/jax-js-nonconsuming`. Ensure the sibling repo `../jax-js-nonconsuming` is present for eslint-plugin linking, or update dependency for isolated debugging.
