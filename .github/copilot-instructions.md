...
- MLE parameter estimation: `src/mle.ts` (`dlmMLE`: autodiff-based MLE via `jit(valueAndGrad + Adam)`, AD-safe `buildDiagW`, `makeKalmanLoss` using `lax.scan`).
