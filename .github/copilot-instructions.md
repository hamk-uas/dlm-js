...
- **Three-branch architecture**: `dlmSmo` selects Float64 (no stabilization), cpu/wasm+Float32 (Joseph form), or webgpu+Float32 (associativeScan+DARE MATLAB convention+Joseph). WASM is ~10–20× faster than CPU for small-matrix dlmFit. WebGPU path uses steady-state approximation (DARE K_ss, MATLAB convention: A_ss = G−K_ss·F, Sigma_base = W); results are correct but ~1–2% less accurate than sequential scan due to early-step gain approximation. **WebGPU is currently always slower than WASM**: jax-js dispatches each op in the compose body as a separate sequential GPU kernel, so the associativeScan runs as O(n × ops_per_compose) dispatches — scaling linearly with N, not O(log n). WASM stays flat at ~22 ms; the WebGPU/WASM ratio doubles with every doubling of N. No crossover exists under the current jax-js execution model.
