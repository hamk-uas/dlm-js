...
- MLE parameter estimation: `src/mle.ts` (`dlmMLE`: autodiff-based MLE via `jit(valueAndGrad + optax Adam)`, AD-safe `buildDiagW`, `buildG` for AR fitting, `makeKalmanLoss` using `lax.scan`). Supports `fitar: true` for AR coefficient estimation.
...
- Tests: `tests/niledemo.test.ts` (Nile demo vs Octave), `tests/gensys.test.ts` (multi-model vs Octave), `tests/synthetic.test.ts` (known true states, statistical assertions), `tests/mle.test.ts` (MLE parameter & AR fitting on WASM).
...
- SVG generators: `scripts/gen-niledemo-svg.ts`, `scripts/gen-kaisaniemi-svg.ts`, `scripts/gen-trigar-svg.ts`, `scripts/gen-nile-mle-svg.ts`, `scripts/gen-energy-mle-anim-svg.ts` (+ `tmp/collect-energy-mle-frames.ts` for frame data).
...
- **Four test suites**: `niledemo.test.ts` (8 tests, Nile data vs Octave), `gensys.test.ts` (47 tests, multi-model vs Octave), `synthetic.test.ts` (24 tests, known true states), `mle.test.ts` (3 tests, MLE parameter & AR fitting on WASM). Total: 82 tests.
...
6. `src/types.ts` (TypedArray shapes, helpers)
