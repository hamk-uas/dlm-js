...
- Upstream issues: `issues/` (precision analysis, WebGPU JIT einsum bug filed to jax-js-nonconsuming).
...
- **Three-branch architecture**: `dlmSmo` selects Float64 (no stabilization), cpu/wasm+Float32 (Joseph form), or webgpu+Float32 (associativeScan+DARE MATLAB convention+Joseph). WASM is ~10–20× faster than CPU for small-matrix dlmFit. WebGPU path uses steady-state approximation (DARE K_ss, MATLAB convention: A_ss = G−K_ss·F, Sigma_base = W); results are correct but ~1–2% less accurate than sequential scan due to early-step gain approximation. **WebGPU is currently always slower than WASM**: `lax.associativeScan` (forward filter) dispatches ⌈log₂N⌉+1 GPU kernels total (one fused fn-kernel per round — architecturally optimal; WebGPU lacks cross-workgroup sync so Kogge-Stone requires a separate dispatch per round); `lax.scan` (backward smoother, inside `jit(core)`) is O(n) dispatches on WebGPU even under `jit` — the step body's intra-step matmul→matmul→add buffer dependencies violate WebGPU's compiled-loop eligibility constraint (no cross-workgroup barrier), so jax-js `planScan` selects `executeScanFallback()`: a JS for-loop issuing one `bodyProgram.execute()` per step. WASM stays flat at ~22 ms (`lax.scan` compiles to a single native module call). No WebGPU/WASM crossover exists. Caveat: for purely elementwise `fn` bodies, `associativeScan` achieves exactly 1 GPU kernel per round; if `fn` contains reductions (matmul, einsum with inner sum), each round produces additional dispatches.
