...
- **Consumer mode (the upgrade & integration loop):** When the user reports a new upstream push to `@hamk-uas/jax-js-nonconsuming`:
  ...
  3. **Run the full test suite**: `pnpm vitest run` — all 236 tests must pass before proceeding.
...
- **Methodology for large tasks:**
  ...
  2. **The ground truth:** Run `pnpm vitest run`. **Test integrity is absolute** — 236 tests across 12 suites, both algorithms, all device × dtype configs.
