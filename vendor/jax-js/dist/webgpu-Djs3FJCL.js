import { AluExp, AluGroup, AluOp, DEBUG, DType, Executable, FpHash, Kernel, Routines, SlotError, UnsupportedOpError, UnsupportedRoutineError, byteWidth, findPow2, isFloatDtype, mapSetUnion, prod, range, strip1, tuneNullopt, tuneWebgpu } from "./backend-BlWLOprI.js";
...
function genScanExpressionWithRidx(exp, dtype, numConsts, numCarry) {
	const gen = (e) => {
		...
		if (op === AluOp.Sub) return `(${gen(src[0])} - ${gen(src[1])})`;
		if (op === AluOp.Mul) {
			if (eDtype === DType.Bool) return `(${gen(src[0])} && ${gen(src[1])})`;
			return `(${gen(src[0])} * ${gen(src[1])})`;
		}
		...
		if (op === AluOp.Reciprocal) return `(1.0 / ${gen(src[0])})`;
		if (op === AluOp.Sqrt) return `sqrt(${gen(src[0])})`;
		if (op === AluOp.Cast) return `${dtypeToWgsl(eDtype)}(${strip1(gen(src[0]))})`;
		if (op === AluOp.Where) return `select(${strip1(gen(src[2]))}, ${strip1(gen(src[1]))}, ${strip1(gen(src[0]))})`;
		if (op === AluOp.Cmplt) return `(${gen(src[0])} < ${gen(src[1])})`;
		if (op === AluOp.Cmpne) return `(${gen(src[0])} != ${gen(src[1])})`;
		if (op === AluOp.Idiv) return isFloatDtype(eDtype) ? `trunc(${gen(src[0])} / ${gen(src[1])})` : `(${gen(src[0])} / ${gen(src[1])})`;
		if (op === AluOp.Mod) return `(${gen(src[0])} % ${gen(src[1])})`;
		...
		if (op === AluOp.Bitcast) return `bitcast<${dtypeToWgsl(eDtype)}>(${strip1(gen(src[0]))})`;
