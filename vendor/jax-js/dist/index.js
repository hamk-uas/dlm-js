...
function tryPrepareNativeScanGeneral(backend, bodyProgram, bodyJaxpr, length, numCarry, numConsts, numX, numY, reverse) {
	if (backend.type !== "wasm") {
		if (DEBUG >= 1) console.log(`[general-scan] skipped, backend=${backend.type} (need wasm)`);
		return null;
	}
	if (DEBUG >= 2) console.log(`[general-scan] trying with backend=${backend.type}, numCarry=${numCarry}, numY=${numY}`);
	const executeSteps = bodyProgram.steps.filter((s) => s.type === "execute");
	...
	for (let i = 0; i < executeSteps.length; i++) {
		...
		for (const inputId of step.inputs) if (inputId < numInputs) inputSlots.push(inputId);
			...
			if (internalIdx === void 0) {
				if (DEBUG >= 1) console.log(`[general-scan] skipped, input ${inputId} not found in slot mapping`);
				return null;
			}
	...
	for (const slot of carryOutSlots) {
		...
		if (internalIdx === void 0) {
			if (DEBUG >= 1) console.log(`[general-scan] skipped, carry output slot ${slot} not produced by any execute step`);
			return null;
		}
		carryOutSources.push({
			type: "internal",
	...
	for (const slot of yOutputSlots) {
		...
		const internalIdx = slotToInternal.get(slot);
		if (internalIdx === void 0) {
			if (DEBUG >= 1) console.log(`[general-scan] skipped, Y output slot ${slot} not found`);
			return null;
		}
		yOutputSources.push({
			type: "internal",
	...
	try {
		...
		if (DEBUG >= 2) console.warn("[general-scan] preparation failed:", e$1);
		return null;
	}
}
