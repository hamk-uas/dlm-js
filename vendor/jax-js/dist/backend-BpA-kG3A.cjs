...
async function createBackend(device) {
	if (device === "cpu") return new CpuBackend();
		...
		const { WebGPUBackend } = await Promise.resolve().then(() => require("./webgpu-D76LRDVX.cjs"));
		...
		const { WebGLBackend } = await Promise.resolve().then(() => require("./webgl-DDiqVHrJ.cjs"));
