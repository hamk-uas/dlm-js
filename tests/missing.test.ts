/**
 * Missing data (NaN observation) tests.
 *
 * Verifies that dlm-js correctly handles NaN observations by comparing
 * against Octave reference output generated by tests/octave/missingdata_test.m.
 *
 * Missing data strategy (matching MATLAB dlmsmo):
 ...
 *     Ensures the smoother does not over-shrink uncertainty near missing obs.
 *
...
if (!fs.existsSync(inFileA) || !fs.existsSync(refFileA) ||
  ...
  throw new Error(
    'Missing data reference files not found — run: pnpm run test:octave:missing'
  );
}
...
// y_missing contains null (from JSON) where observations are NaN
...
describe('missing data (NaN observations) — order=1 (m=2)', async () => {
  const configs = await getTestConfigs();

  for (const config of configs) {
    it(`should match Octave reference (${config.label})`, async () => {
      await runTest(config, y_missing as number[], s, w, { order: 1 }, refA, 'order1', 2);
    });
...
describe('missing data (NaN observations) — order=0 (m=1)', async () => {
  const configs = await getTestConfigs();

  for (const config of configs) {
    it(`should match Octave reference (${config.label})`, async () => {
      await runTest(config, y_missing as number[], s, w_level, { order: 0 }, refB, 'order0', 1);
    });
