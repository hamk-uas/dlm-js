/**
 * UD (Bierman/Thornton) filter + smoother validation against Octave references.
 *
 * The UD path factors C = U·D·U' (unit lower-triangular U, positive diagonal D)
 * and uses Bierman's scalar measurement update + Thornton's modified weighted
 * Gram-Schmidt (MWGS) time update.  Only p=1 (scalar observations) is supported.
 *
 * These tests force algorithm='ud' and validate against the same Octave ground
 * truth used by gensys.test.ts and assocscan.test.ts.
 *
    ...
    describe(`ud / ${config.label}`, () => {
      for (const mc of modelCases) {
        it(mc.name, async () => {
          ...
          const result = await dlmFit(input.y, { obsStd: input.s, processStd: w, dtype: getDlmDtype(config), ...mc.options, algorithm: 'ud' });
          ...
          fs.writeFileSync(
            path.join(outputDir, mc.referenceFile.replace('-m.json', `-ud-${config.label.replace('/', '-')}.json`)),
          ...
          if (!cmp.equal) {
            throw new Error(
              `[ud / ${config.label} | ${mc.name}] Mismatch at: ${cmp.path}\n` +
...
// ── Niledemo: UD vs Octave ─────────────────────────────────────────────────

describe('ud niledemo vs Octave', async () => {
  const configs = await getTestConfigs();
  const f64Configs = configs.filter(c => c.label.includes('f64'));
  ...
  for (const config of f64Configs) {
    it(`should match reference — ud (${config.label})`, async () => {
      applyConfig(config);

      const result = await dlmFit(nileInput.y, { obsStd: nileInput.s, processStd: nileInput.w, dtype: getDlmDtype(config), algorithm: 'ud' });
      ...
      if (!fs.existsSync(outputDir)) fs.mkdirSync(outputDir, { recursive: true });
      fs.writeFileSync(
        path.join(outputDir, `niledemo-out-ud-${config.label.replace('/', '-')}.json`),
        JSON.stringify(matlab, (_key, value) =>
          ArrayBuffer.isView(value) ? Array.from(value as Float64Array) : value
        , 2)
      );
      ...
      if (!cmp.equal) {
        throw new Error(
          `[ud / ${config.label}] Niledemo mismatch at: ${cmp.path}\n` +
          `Result:    ${JSON.stringify(cmp.a)}\n` +
          `Reference: ${JSON.stringify(cmp.b)}`
        );
      }
...
// ── Missing data: UD vs Octave ─────────────────────────────────────────────

describe('ud gapped data vs Octave', async () => {
  const configs = await getTestConfigs();
  const f64Configs = configs.filter(c => c.label.includes('f64'));
  ...
  for (const config of f64Configs) {
    it(`order=1 (m=2) should match Octave — ud (${config.label})`, async () => {
      applyConfig(config);

      const result = await dlmFit(
        Float64Array.from(y_gapped.map(v => (v === null ? NaN : v))),
        { obsStd: s, processStd: w, dtype: getDlmDtype(config), order: 1, algorithm: 'ud' },
      );
      ...
      if (!cmp.equal) {
        throw new Error(
          `[ud / ${config.label}] Gapped order=1 mismatch at: ${cmp.path}\n` +
          `Result:    ${JSON.stringify(cmp.a)}\n` +
          `Reference: ${JSON.stringify(cmp.b)}`
        );
      }
    ...
    it(`order=0 (m=1) should match Octave — ud (${config.label})`, async () => {
      applyConfig(config);

      const result = await dlmFit(
        Float64Array.from(y_gapped.map(v => (v === null ? NaN : v))),
        { obsStd: s, processStd: [w_level], dtype: getDlmDtype(config), order: 0, algorithm: 'ud' },
      );
      ...
      if (!cmp.equal) {
        throw new Error(
          `[ud / ${config.label}] Gapped order=0 mismatch at: ${cmp.path}\n` +
          `Result:    ${JSON.stringify(cmp.a)}\n` +
          `Reference: ${JSON.stringify(cmp.b)}`
        );
      }
...
// ── Float32 smoke tests: all outputs finite ────────────────────────────────

describe('ud f32 dlmFit (smoke: all outputs finite)', async () => {
  const configs = await getTestConfigs();
  const f32Configs = configs.filter(c => c.label.includes('f32'));

  // UD (Bierman/Thornton) does not apply Joseph form + cEps stabilization,
  // so f32 + large m may diverge.  Skip known-NaN combos.
  // Energy demand (energy-in.json, AR(1) coeff=0.85, m=5) diverges on WASM
  // f32: 119/120 NaN from t=1 onward.  CPU f32 UD stays stable (~9e-3 rel err).
  ...
  for (const config of f32Configs) {
    describe(`ud-f32 / ${config.label}`, () => {
      for (const mc of modelCases) {
        const skip = SKIP_F32[config.label]?.includes(mc.name);
        (skip ? it.skip : it)(mc.name, async () => {
          applyConfig(config);
          ...
          if (!fs.existsSync(outputDir)) fs.mkdirSync(outputDir, { recursive: true });
          fs.writeFileSync(
            path.join(outputDir, mc.referenceFile.replace('-m.json', `-ud-f32-${config.label.replace('/', '-')}.json`)),
            JSON.stringify(matlab, (_key, value) =>
              ArrayBuffer.isView(value) ? Array.from(value as Float64Array) : value
            , 2)
          );
...
describe('ud f32 niledemo (smoke: all outputs finite)', async () => {
  const configs = await getTestConfigs();
  const f32Configs = configs.filter(c => c.label.includes('f32'));
  ...
  for (const config of f32Configs) {
    it(`all outputs finite — ud-f32 (${config.label})`, async () => {
      applyConfig(config);
...
describe('ud f32 gapped data (smoke: all outputs finite)', async () => {
  const configs = await getTestConfigs();
  const f32Configs = configs.filter(c => c.label.includes('f32'));
  ...
  for (const config of f32Configs) {
    it(`order=1 (m=2) all outputs finite — ud-f32 (${config.label})`, async () => {
      applyConfig(config);
      const result = await dlmFit(
        Float64Array.from(y_gapped.map(v => (v === null ? NaN : v))),
        { obsStd: s, processStd: w, dtype: getDlmDtype(config), order: 1, algorithm: 'ud' },
    ...
    it(`order=0 (m=1) all outputs finite — ud-f32 (${config.label})`, async () => {
      applyConfig(config);
      const result = await dlmFit(
        Float64Array.from(y_gapped.map(v => (v === null ? NaN : v))),
        { obsStd: s, processStd: [w_level], dtype: getDlmDtype(config), order: 0, algorithm: 'ud' },
