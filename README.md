...
- **WASM stays flat up to N≈3200, then scales linearly** (\~<!-- timing:scale:wasm-f64:n1638400 -->1431 ms (warm)<!-- /timing --> at N=1.6M). WebGPU scales sub-linearly at small N but approaches O(N) at large N (<!-- timing:scale:webgpu-f32:n100 -->1295 ms (cold)<!-- /timing --> → <!-- timing:scale:webgpu-f32:n102400 -->2485 ms (cold)<!-- /timing --> for a 1024× increase). No crossover was observed up to N=1.6M; see scaling table.
...
|-------|-----|-----|--------------------------|------------------------------|
| Nile, order=0 | 100 | 1 | <!-- timing:bb:nile-o0:wasm-f64 -->6 ms<!-- /timing --> | <!-- timing:bb:nile-o0:webgpu-f32 -->39 ms<!-- /timing --> |
| Nile, order=1 | 100 | 2 | <!-- timing:bb:nile-o1:wasm-f64 -->6 ms<!-- /timing --> | <!-- timing:bb:nile-o1:webgpu-f32 -->35 ms<!-- /timing --> |
| Kaisaniemi, trig | 117 | 4 | <!-- timing:bb:kaisaniemi:wasm-f64 -->6 ms<!-- /timing --> | <!-- timing:bb:kaisaniemi:webgpu-f32 -->62 ms<!-- /timing --> |
| Energy, trig+AR | 120 | 5 | <!-- timing:bb:trigar:wasm-f64 -->6 ms<!-- /timing --> | <!-- timing:bb:trigar:webgpu-f32 -->64 ms<!-- /timing --> |
...
|---|----------------------|-------------------------|-------|
| 100 | <!-- timing:scale:wasm-f64:n100 -->7 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n100 -->1295 ms<!-- /timing --> | 181× |
| 200 | <!-- timing:scale:wasm-f64:n200 -->6 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n200 -->1306 ms<!-- /timing --> | 227× |
| 400 | <!-- timing:scale:wasm-f64:n400 -->6 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n400 -->1650 ms<!-- /timing --> | 272× |
| 800 | <!-- timing:scale:wasm-f64:n800 -->7 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n800 -->1641 ms<!-- /timing --> | 247× |
| 1600 | <!-- timing:scale:wasm-f64:n1600 -->7 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n1600 -->1626 ms<!-- /timing --> | 228× |
| 3200 | <!-- timing:scale:wasm-f64:n3200 -->8 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n3200 -->1717 ms<!-- /timing --> | 220× |
| 6400 | <!-- timing:scale:wasm-f64:n6400 -->11 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n6400 -->1863 ms<!-- /timing --> | 179× |
| 12800 | <!-- timing:scale:wasm-f64:n12800 -->17 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n12800 -->1998 ms<!-- /timing --> | 128× |
| 25600 | <!-- timing:scale:wasm-f64:n25600 -->33 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n25600 -->2023 ms<!-- /timing --> | 79× |
| 51200 | <!-- timing:scale:wasm-f64:n51200 -->55 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n51200 -->2223 ms<!-- /timing --> | 50× |
| 102400 | <!-- timing:scale:wasm-f64:n102400 -->83 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n102400 -->2485 ms<!-- /timing --> | 27× |
| 204800 | <!-- timing:scale:wasm-f64:n204800 -->172 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n204800 -->2850 ms<!-- /timing --> | 20× |
| 409600 | <!-- timing:scale:wasm-f64:n409600 -->339 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n409600 -->3796 ms<!-- /timing --> | 12× |
| 819200 | <!-- timing:scale:wasm-f64:n819200 -->698 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n819200 -->6019 ms<!-- /timing --> | 9.1× |
| 1638400 | <!-- timing:scale:wasm-f64:n1638400 -->1431 ms<!-- /timing --> | <!-- timing:scale:webgpu-f32:n1638400 -->9792 ms<!-- /timing --> | 7.4× |

Three findings:

1. **WASM stays flat up to N≈3200**, then grows roughly linearly (O(n)). The per-step cost asymptotes around \~0.8 µs/step (<!-- timing:scale:wasm-f64:n1638400 -->1431 ms (warm)<!-- /timing --> at N=1638400). The flat region reflects fixed JIT/dispatch overhead, not compute. WASM OOM previously occurred at N=3276800 due to a signed 32-bit integer overflow in the page-count calculation — fixed upstream in jax-js-nonconsuming (bb126c6+a6d37da).

2. **WebGPU cold timings include per-N JIT recompilation** (WebGPU `jit()` is not polymorphic in N). At small N the \~1200 ms JIT overhead dominates; at large N the GPU arithmetic dominates. Each associativeScan pass dispatches ⌈log₂N⌉+1 Kogge-Stone rounds, each operating on all N elements in parallel — so total GPU work is O(N log N), while WASM sequential scan is O(N). A 1024× increase from N=100 to N=102400 roughly doubles the runtime (<!-- timing:scale:webgpu-f32:n100 -->1295 ms (cold)<!-- /timing --> → <!-- timing:scale:webgpu-f32:n102400 -->2485 ms (cold)<!-- /timing -->).
...
**Performance**: on the `wasm` backend, one Nile MLE run (100 observations, m = 2) converges in <!-- timing:mle-bench:nile-order1:iterations -->190<!-- /timing --> iterations (\~<!-- timing:mle-bench:nile-order1:elapsed -->39 ms<!-- /timing -->) with Adam (b2=0.9), or <!-- timing:nat-mle-bench:nile-order1:iterations -->15<!-- /timing --> iterations (\~<!-- timing:nat-mle-bench:nile-order1:elapsed -->23 ms<!-- /timing -->) with the natural gradient optimizer.
...
| **WASM performance** | Adam: <!-- timing:mle-bench:nile-order1:elapsed -->39 ms<!-- /timing --> / <!-- timing:mle-bench:nile-order1:iterations -->190<!-- /timing --> iters; natural: <!-- timing:nat-mle-bench:nile-order1:elapsed -->23 ms<!-- /timing --> / <!-- timing:nat-mle-bench:nile-order1:iterations -->15<!-- /timing --> iters (Nile, n=100, m=2, incl. JIT) | N/A |
...
|-------|---|---|--------|--------------------|-----------------------|---------------------|-------------------|----------------------|---------------------|
| Nile, order=1, fit s+w | 100 | 2 | 3 | <!-- timing:mle-bench:nile-order1:elapsed -->39 ms<!-- /timing --> | <!-- timing:nat-mle-bench:nile-order1:elapsed -->23 ms<!-- /timing --> | <!-- computed:static("octave-nile-order1-elapsed-ms") + " ms" -->2814 ms<!-- /computed --> | <!-- timing:mle-bench:nile-order1:lik -->1104.9<!-- /timing --> | <!-- timing:nat-mle-bench:nile-order1:lik -->1104.9<!-- /timing --> | <!-- computed:static("octave-nile-order1-lik") -->1104.6<!-- /computed --> |
...
| Nile, order=0, fit s+w | 100 | 1 | 2 | <!-- timing:mle-bench:nile-order0:elapsed -->21 ms<!-- /timing --> | <!-- timing:nat-mle-bench:nile-order0:elapsed -->13 ms<!-- /timing --> | <!-- computed:static("octave-nile-order0-elapsed-ms") + " ms" -->607 ms<!-- /computed --> | <!-- timing:mle-bench:nile-order0:lik -->1095.8<!-- /timing --> | <!-- timing:nat-mle-bench:nile-order0:lik -->1095.8<!-- /timing --> | <!-- computed:static("octave-nile-order0-lik") -->1095.8<!-- /computed --> |
| Kaisaniemi, trig, fit s+w | 117 | 4 | 5 | <!-- timing:mle-bench:kaisaniemi:elapsed -->86 ms<!-- /timing --> | <!-- timing:nat-mle-bench:kaisaniemi:elapsed -->74 ms<!-- /timing --> | **failed** (NaN/Inf) | <!-- timing:mle-bench:kaisaniemi:lik -->341.3<!-- /timing --> | <!-- timing:nat-mle-bench:kaisaniemi:lik -->341.3<!-- /timing --> | — |
| Energy, trig+AR, fit s+w+φ | 120 | 5 | 7 | <!-- timing:energy-mle:elapsed-ms -->284 ms<!-- /timing --> | <!-- timing:nat-mle-bench:energy:elapsed -->139 ms<!-- /timing --> | — | <!-- timing:energy-mle:lik -->443.1<!-- /timing --> | <!-- timing:nat-mle-bench:energy:lik -->443.1<!-- /timing --> | — |
...
|---------|---|---|-------------------------------|---------------------------------------|---------|
| Nile, order=1 | 100 | 2 | <!-- timing:ckpt:nile:false-ms -->24 ms<!-- /timing --> | <!-- timing:ckpt:nile:true-ms -->20 ms<!-- /timing --> | <!-- timing:ckpt:nile:speedup -->-19%<!-- /timing --> |
| Energy, order=1+trig1+ar1 | 120 | 5 | <!-- timing:ckpt:energy:false-ms -->37 ms<!-- /timing --> | <!-- timing:ckpt:energy:true-ms -->36 ms<!-- /timing --> | <!-- timing:ckpt:energy:speedup -->-5%<!-- /timing --> |
