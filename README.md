...
**Performance**: on the `wasm` backend, one Nile MLE run (100 observations, m = 2) converges in <!-- timing:mle-bench:nile-order1:iterations -->190<!-- /timing --> iterations (~<!-- timing:mle-bench:nile-order1:elapsed -->3322 ms<!-- /timing -->) with Adam (b2=0.9), or <!-- timing:nat-mle-bench:nile-order1:iterations -->5<!-- /timing --> iterations (~<!-- timing:nat-mle-bench:nile-order1:elapsed -->1520 ms<!-- /timing -->) with the natural gradient optimizer.
...
The MATLAB DLM toolbox was run under Octave with `fminsearch` (Nelder-Mead, `maxfuneval=400` for Nile models, `maxfuneval=800` for Kaisaniemi). dlm-js uses `dlmMLE` with two optimizers: **Adam** (first-order, `maxIter=300`, `b2=0.9`, `checkpoint: false`) and **natural gradient** (second-order LM-damped Fisher scoring with FD Hessian, `maxIter=50`), both on `wasm` backend, `Float64`. Octave timings are median of 5 runs after 1 warmup on <!-- computed:static("_note").match(/on an? ([^.]+?) after/)?.[1] ?? "unknown machine"-->AMD Ryzen 5 5600H<!-- /computed --> (measured <!-- computed:static("_measured")-->2025-01<!-- /computed -->); dlm-js timings are single fresh-run wall-clock times (including first-call JIT overhead) on <!-- computed:static("machine")-->Intel(R) Core(TM) Ultra 5 125H, 62 GB RAM<!-- /computed -->. **Note:** Octave and dlm-js timings are from different machines — timing ratios are approximate. Re-run `pnpm run test:octave` and update `static-references.json` to get same-machine numbers.

| Model | $n$ | $m$ | params | dlm-js Adam (wasm) | dlm-js natural (wasm) | Octave `fminsearch` | $-2\log L$ (Adam) | $-2\log L$ (natural) | $-2\log L$ (Octave) |
|-------|---|---|--------|--------------------|-----------------------|---------------------|-------------------|----------------------|---------------------|
| Nile, order=1, fit s+w | 100 | 2 | 3 | <!-- timing:mle-bench:nile-order1:elapsed -->3322 ms<!-- /timing --> | <!-- timing:nat-mle-bench:nile-order1:elapsed -->1520 ms<!-- /timing --> | <!-- computed:static("octave-nile-order1-elapsed-ms") + " ms" -->2827 ms<!-- /computed --> | <!-- timing:mle-bench:nile-order1:lik -->1104.9<!-- /timing --> | <!-- timing:nat-mle-bench:nile-order1:lik -->1109.3<!-- /timing --> | <!-- computed:static("octave-nile-order1-lik") -->1104.6<!-- /computed --> |
...
| Nile, order=0, fit s+w | 100 | 1 | 2 | <!-- timing:mle-bench:nile-order0:elapsed -->2328 ms<!-- /timing --> | <!-- timing:nat-mle-bench:nile-order0:elapsed -->1270 ms<!-- /timing --> | <!-- computed:static("octave-nile-order0-elapsed-ms") + " ms" -->610 ms<!-- /computed --> | <!-- timing:mle-bench:nile-order0:lik -->1095.8<!-- /timing --> | <!-- timing:nat-mle-bench:nile-order0:lik -->1095.8<!-- /timing --> | <!-- computed:static("octave-nile-order0-lik") -->1095.8<!-- /computed --> |
| Kaisaniemi, trig, fit s+w | 117 | 4 | 5 | <!-- timing:mle-bench:kaisaniemi:elapsed -->5505 ms<!-- /timing --> | <!-- timing:nat-mle-bench:kaisaniemi:elapsed -->4653 ms<!-- /timing --> | **failed** (NaN/Inf) | <!-- timing:mle-bench:kaisaniemi:lik -->341.3<!-- /timing --> | <!-- timing:nat-mle-bench:kaisaniemi:lik -->341.3<!-- /timing --> | — |
| Energy, trig+AR, fit s+w+φ | 120 | 5 | 7 | <!-- timing:energy-mle:elapsed-ms -->6362 ms<!-- /timing --> | — | — | <!-- timing:energy-mle:lik -->443.1<!-- /timing --> | — | — |

Octave timings are from Octave with `fminsearch`; Nile and Kaisaniemi dlm-js timings are from `pnpm run bench:mle`; Energy dlm-js timings are from `collect-energy-mle-frames.ts` (includes frame extraction overhead). All dlm-js timings are single fresh-run wall-clock times including JIT overhead.

**Key observations:**
- **Nile (n=100, m=2):** Octave `fminsearch` is <!-- computed:static("octave-nile-order1-elapsed-ms") < slot("mle-bench:nile-order1:elapsed") ? "faster" : "slower" -->faster<!-- /computed --> (see table). dlm-js includes one-time JIT compilation overhead in the reported time. Note: Octave and dlm-js were measured on different machines (see note above).
...
- **Kaisaniemi (m=4, 5 params):** Octave `fminsearch` (`maxfuneval=800`) failed with NaN/Inf; Adam converged in <!-- timing:mle-bench:kaisaniemi:iterations -->300<!-- /timing --> iterations (~<!-- timing:mle-bench:kaisaniemi:elapsed-s -->5.5 s<!-- /timing -->), natural gradient in <!-- timing:nat-mle-bench:kaisaniemi:iterations -->22<!-- /timing --> iterations (~<!-- timing:nat-mle-bench:kaisaniemi:elapsed-s -->4.7 s<!-- /timing -->).
